...
def interactive_demo():
    ...
    # デバイスの選択（設定ファイルから読み込むか、ユーザーに選択させる）
    device_index = config.get("device_index")
    device_name = "デフォルト" if device_index is None else next((d["name"] for d in devices if d["index"] == device_index), "不明")

    if device_index is not None:
        print(f"\n前回の設定から選択されたオーディオデバイス: {device_name} (index: {device_index})")
        use_saved = input("この設定を使用しますか？ (y/n, デフォルトはy): ").lower() != 'n'
        if not use_saved:
            device_index = None
    ...
    # スピーカーIDの選択（設定ファイルから読み込むか、ユーザーに選択させる）
        ...
        for speaker in speakers:
            for style in speaker["styles"]:
                if style["id"] == speaker_id:
                    speaker_name = f"{speaker['name']} - {style['name']}"
                    break
            if speaker_name:
                break

        print(f"\n前回の設定から選択されたスピーカー: {speaker_name} (ID: {speaker_id})")
        use_saved = input("この設定を使用しますか？ (y/n, デフォルトはy): ").lower() != 'n'
        if not use_saved:
            speaker_id = None
    ...
    if speaker_id is None:
        if speakers:
            ...
            try:
                ...
                speaker_name = available_styles[int(choice)][1]
            except (ValueError, IndexError):
                if speakers and speakers[0]["styles"]:
                    speaker_id = speakers[0]["styles"][0]["id"]
                    speaker_name = f"{speakers[0]['name']} - {speakers[0]['styles'][0]['name']}"
                    print(f"デフォルトスピーカーを使用します: {speaker_name}")
                else:
...
def websocket_client_demo():
    ...
    # WebSocket設定を読み込む
    ws_url = config.get("ws_url", "ws://127.0.0.1:2231")
    saved_ws_url = ws_url

    # URLの変更オプション
    if saved_ws_url:
        print(f"\n前回の設定から選択されたWebSocketサーバー: {saved_ws_url}")
        use_saved_url = input("この設定を使用しますか？ (y/n, デフォルトはy): ").lower() != 'n'
        if not use_saved_url:
            ws_url = input("新しいWebSocketサーバーURLを入力してください: ")
            if not ws_url:

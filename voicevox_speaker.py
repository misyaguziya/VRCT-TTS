...
class VoicevoxSpeaker:
    ...
    @staticmethod
    def list_audio_devices() -> List[Dict[str, Union[int, str]]]:
        """
        利用可能なオーディオデバイスの一覧を取得する
        
        Returns:
            List[Dict[str, Union[int, str]]]: デバイス情報のリスト
        """
        p = pyaudio.PyAudio()
        devices = []
...
if __name__ == "__main__":
    ...
    if devices:
        speaker = VoicevoxSpeaker(output_device_index=devices[0]['index'])
        ...
        if vv_speakers:
            speaker_id = vv_speakers[0]["styles"][0]["id"]
            
            # クエリを作成して音声合成
            query = client.audio_query("こんにちは、テストです。", speaker_id)
